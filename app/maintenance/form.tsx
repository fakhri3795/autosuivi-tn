...
import { InputField, GradientButton, GlassCard } from '../../src/presentation/components';
...
export default function MaintenanceFormScreen() {
  ...
  const { activeVehicle, maintenanceRecords, addMaintenanceRecord, updateMaintenanceRecord } = useVehicle();
  const existingRecord = maintenanceRecords.find((record) => record.id === maintenanceId);
  const isEditing = !!existingRecord;
  const [saving, setSaving] = useState(false);
  const [selectedType, setSelectedType] = useState<MaintenanceType>(
    existingRecord?.type ?? MaintenanceType.VIDANGE,
  );
  ...
  const [formData, setFormData] = useState({
    mileage: existingRecord?.mileage?.toString() ?? activeVehicle?.currentMileage?.toString() ?? '',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{isEditing ? 'Modifier Maintenance' : 'Nouvelle Maintenance'}</Text>
