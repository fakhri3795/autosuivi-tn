...
import { formatDateFr } from '../../src/core/utils/dateUtils';
...
export default function MaintenanceScreen() {
  ...
  const renderUpcomingItem = ({ item }: { item: PredictionItem }) => {
    ...
    const formattedDate = formatDateFr(item?.estimatedDate);
      ...
      <View style={styles.upcomingItem}>
        ...
        <View style={styles.upcomingInfo}>
          ...
          <Text style={styles.upcomingDate}>
            Prochain : {(item?.estimatedKm ?? 0).toLocaleString()} km • {formattedDate}
          </Text>
        </View>
        ...
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Erreur', 'Impossible de supprimer cette maintenance.');
            }
          },
        },
      ],
  ...
  const renderHistoryItem = ({ item }: { item: MaintenanceRecord }) => (
    <MaintenanceItem
      record={item}
      onEdit={() => router.push(`/maintenance/form?maintenanceId=${item.id}`)}
      onDelete={() => handleDeleteMaintenance(item)}
    />
  );
