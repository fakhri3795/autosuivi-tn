...
import { parseAppDate } from '../../core/utils/dateUtils';
...
export const calculateNextOilChange = (
  ...
  const intervalMonths = customIntervals?.oilChangeMonths ?? DefaultIntervals[MaintenanceType.VIDANGE].months;
  ...
  const kmBasedDays = Math.max(0, Math.round(kmRemaining / effectiveAvgKmPerDay));
  
  const now = new Date();
  const kmBasedDate = new Date(now);
  kmBasedDate.setDate(kmBasedDate.getDate() + kmBasedDays);

  const lastMaintenanceDate = parseAppDate(lastOilChange?.date);
  const timeBasedDate = lastMaintenanceDate ? new Date(lastMaintenanceDate) : new Date(now);
  timeBasedDate.setMonth(timeBasedDate.getMonth() + intervalMonths);

  const estimatedDate = timeBasedDate.getTime() < kmBasedDate.getTime() ? timeBasedDate : kmBasedDate;
  const daysRemaining = Math.max(
    0,
    Math.ceil((estimatedDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
  );
...
export const getUpcomingMaintenanceItems = (
  ...
  for (const type of Object.values(MaintenanceType)) {
    ...
    const kmRemaining = Math.max(0, nextKm - currentMileage);
    const kmBasedDays = Math.max(0, Math.round(kmRemaining / Math.max(stats?.avgKmPerDay ?? 30, 1)));
    
    const now = new Date();
    const kmBasedDate = new Date(now);
    kmBasedDate.setDate(kmBasedDate.getDate() + kmBasedDays);
    const lastMaintenanceDate = parseAppDate(lastOfType?.date);
    const timeBasedDate = lastMaintenanceDate ? new Date(lastMaintenanceDate) : new Date(now);
    timeBasedDate.setMonth(timeBasedDate.getMonth() + (interval?.months ?? 12));
    const estimatedDate = timeBasedDate.getTime() < kmBasedDate.getTime() ? timeBasedDate : kmBasedDate;
    ...
    items.push({
      ...
      estimatedKm: nextKm,
      estimatedDate: estimatedDate.toISOString(),
      lastMaintenanceKm: lastOfType?.mileage ?? 0,
    });
  }
